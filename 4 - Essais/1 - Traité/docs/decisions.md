...
| [NF-05 — débit](../bancs/nf05-debit/VERDICT.md) | **[M] Cible non atteinte** : de l'ordre de 20 à 25 s simulées/s-cœur à n = 1 000 contre 10³ (remesuré le 4 septembre 2026 après la phase 2 de `audit.md`, qui a doublé le débit ; 10 à 15 avant elle, et 0,2–2,0 en phase 1). ⚠ *Cette ligne attribuait le rattrapage de la phase 1 à la rétention ; corrigé le 4 septembre 2026 : `appliquer_retention` n'a aucun appelant hors test, et le gain vient de la recherche binaire de `Partition::valider` et de `Milieu::lire`* | L'écart est structurel — chaque agent lit ce que toute la population écrit, donc Θ(n²). **La cible est à refaire sur la mesure**, comme DT1 l'a été. Rien de la phase 1 n'en dépend |
| Vérification du dépôt — 17 août 2026 | **[M]** Dix agents, cinq morceaux, deux tours chacun (bâtisseur + critique en contexte neuf). Suite verte à **465 tests** ; clippy et rustdoc à 0. Le banc n'a pas de `VERDICT.md` : ses pièces étaient les cinq rapports de morceau et les cinq de critique d'un dossier de banc sorti du dépôt le 25 août 2026, et sa consolidation en était le verdict | Le défaut le plus fréquent trouvé n'est pas un défaut de code : c'est **un chiffre écrit sans la commande qui le produit**. D'où la règle appliquée depuis à tous les documents — un compte se donne avec sa ligne de mesure et sa date, jamais gravé. Trois conséquences normatives : DT5 amendé sur l'édition, quatre décisions de conception ouvertes ci-dessous, et le reclassement de deux écarts au traité |
...
Des choix pris **dans la vue** faute de pouvoir les prendre ailleurs. Chacun se
rouvrait par un changement de `sim-agents`, listé au §0 du PRD. ☑ **Trois sont
fermés depuis le 4 septembre 2026**, par la phase 4 du plan d'exécution de
[`audit.md`](../audit.md), qui a fait ces changements ; le premier reste ouvert.

| Décision | Marque | État |
|---|---|---|
...
| **La sentinelle `+∞` est traduite dans la vue, pas dans le type** | **[C]** — ☑ *fermée le 4 septembre 2026* | `Mesures::plancher_observe` et `hors_dominante_observee` valaient `f64::INFINITY` tant qu'aucun cycle ne les avait mis à jour, et `sim-viz` rendait « jamais observé » au lieu de `inf`. **Le changement de type est fait** : les deux champs sont des `Option<f64>`, le `Default` dérivé donne `None`, `verifier_bornes` met à jour par `map_or`, et c'est le type qui oblige l'écran à choisir un libellé. La traduction de la vue reste — deux sites d'appel, et son rustdoc porte la raison F2 —, mais elle prend une absence au lieu d'une sentinelle. Le banc de parité absorbe `unwrap_or(f64::INFINITY)`, pour que les six empreintes ne bougent pas sur un changement de forme d'API |
| **Le vainqueur en temps est lu dans une chaîne française** | **[R]** — ☑ *fermée le 4 septembre 2026* | `Comparaison::verdict_temps` ne rendait le vainqueur que dans une phrase formatée, que la figure du croisement lisait par `contains("la maille gagne")`. **`qui_gagne_en_temps() -> Vainqueur` existe**, et `verdict_temps` compose sa phrase à partir d'elle : le vainqueur est une donnée, la phrase son rendu. `VAINQUEUR_MAILLE` et son test de garde de `sim-viz` disparaissent, remplacés dans `sim-agents` par un test que la phrase nomme le même vainqueur que l'énumération, sur les deux régimes |
| **La provenance des deux bornes est recopiée dans la vue** | **[R]** — ☑ *fermée le 4 septembre 2026* | `scenario_b.rs::SOURCE_BORNES` portait « §1.2, p. 16, 4ᵉ éd. » faute d'accesseur, et la copie avait déjà divergé de `Bornes::LEGENDE` affichée quatre lignes plus bas dans le même cadre — deux pages pour une seule source, le défaut exact que F2 vise. **`Bornes::SOURCE` existe** ; `LEGENDE` garde sa page en toutes lettres, une citation ne se composant pas, et la garde change de crate au lieu de disparaître : `la_legende_porte_la_provenance`, dans `sim-agents` |

## Les décisions ouvertes par le banc du 17 août 2026

Quatre décisions que les bâtisseurs du banc ont **laissées ouvertes plutôt que
...
vérification de crate ne doit pas trancher seul.

| Décision ouverte | Ce que la mesure établit | Ce qu'il faudrait pour la trancher |
|---|---|---|
| **Fermer `Proprietes` par le type, ou renoncer à tenir PD12 par le type** (M1, C1) | `#[non_exhaustive]` interdit le littéral, **pas l'affectation de champ**. `sim_core::detecteur::Proprietes` est `Copy` avec quatre champs `pub`, et `Detecteur::proprietes()` en rend un exemplaire par valeur : depuis une crate externe, un exemplaire à `suspicions = 1 000 000, fausses = 0, exactitude = Some(0.0)` se fabrique. Ce que PD12 tient quand même est plus étroit qu'annoncé — le détecteur ne prend ces nombres que de `sonder`, donc une copie falsifiée ne trompe qu'un affichage. La doc de `detecteur.rs` porte désormais la mesure au lieu de l'affirmation contraire | Champs privés derrière quatre accesseurs. **Casse `crates/sim-agents/src/pair_a_pair.rs`**, qui lit `p.suspicions` et `p.fausses_suspicions` : c'est un changement d'interface entre deux crates, donc une décision de conception, pas un correctif de banc |
| **Remonter dans `sim-agents` les trois valeurs de paramètre encore transcrites dans `sim-viz`** (M5, E6) — ☑ *les six du scénario A sont remontées le 4 septembre 2026* | `VueA::default` pose six valeurs — `n: 64, p: 8, l99_ms: 20.0, aller_simple_ms: 2.0, degre_depot: 3, taux_omission: 0.01` — et `VueB::default` trois — `n: 16, budget: 150_000, graine: 1`. Aucune ne vient de `sim-agents` (`grep -rn '150_000' crates/sim-agents/src/` ne rend rien). Elles ne sont pas sans provenance pour autant : ce sont **les défauts des tableaux du §7 du PRD**, transcrits faute d'accesseur — sauf la graine, qui ne figure dans aucun tableau. Rien ne tient la transcription en accord, et un test qui comparerait la vue à elle-même serait tautologique | ☑ **Le constructeur de défaut de `scenario_a` est fait** : `ParamsA` porte les six valeurs du §7 avec leur provenance champ par champ, `scenario_a` prend une structure au lieu de sept paramètres positionnels, et `VueA` en garde un exemplaire — qui sert aussi de clé au cache de l'écran. Reste **le défaut nommé de `VueB`**, ses trois valeurs, et le contrat de `sim-viz` porte donc « zéro définition de scénario **à une exception nommée** » au lieu d'un absolu : c'est vrai, et plus faible. Pour fermer : `Params::scenario_b()` pose n = 64 là où la vue ouvre à 16 pour ne pas payer seize fois le premier tracé (Θ(n²)) — c'est un choix de coût d'ouverture, pas un défaut de scénario, et il demande donc un nom, pas un accesseur |
| **`ModeleFaute::avertissements` n'a aucun appelant — donc la moitié `[U]` d'EX-C06 n'est tenue par personne** (M1, C4 et D2) | Vérifié : aucun appelant dans `crates/`, `bancs/` compris. Les avertissements que `sim-agents` et `sim-viz` affichent viennent de `sim_agents::stigmergie::Params::avertissements`, homonyme et sans rapport. Le banc a **ajouté** à cette fonction l'avertissement de crash corrélé que le passage des taux par niveau rendait nécessaire — mesuré : un centre unique à 0,09 vide toute la population 9,3 % des pas, contre 0 vidage complet pour un `crash_machine` de même espérance par acteur —, et cet avertissement neuf hérite de la même réserve : calculé, affiché par personne | Un point d'appel dans la vue, ou le retrait d'EX-C06 de sa moitié `[U]`. En attendant, la réserve est déclarée par `ModeleFaute::hors_modele()` et par le §0 du PRD |
| **Ouvrir `sim_core::hors_perimetre()`, ou assumer que le cœur n'en a pas** (M1, R2) | `grep -rn 'fn hors_perimetre' crates/` rend deux fonctions, `sim-milieu` et `sim-agents` ; **`sim-core` n'en a aucune**. Les absences du cœur logent donc dans `ModeleFaute::hors_modele()`, dont ce n'est pas l'objet : sa première entrée y énumère **neuf** mécanismes sans appelant — `tirer_pannes`, `Moteur::avancer_partition`, `message_perdu`, `injection_echec`, `injection_retard`, `injection_valeur`, `retard_message`, `ecriture_corrompue`, `avertissements` —, dont six n'ont aucun rapport avec le modèle de faute pris comme modèle. Le plancher mémoire d'EX-C17, EX-C08 et EX-C16 y sont aussi, ou nulle part | Trancher : soit une quatrième liste vivante, ce qui touche `docs/PRD.md`, `docs/SPEC.md`, `CLAUDE.md` et l'onglet « Limites » (une septième section) ; soit écrire que `hors_modele()` est **par convention** la liste d'absences du cœur, ce qui coûte une phrase et laisse le nom mentir. **Le code n'est le périmètre d'aucune des deux vérifications** ; la décision est ici, la fonction n'est pas créée |

## Les décisions prises par l'exécution de l'audit du 4 septembre 2026

Deux décisions que le plan de [`audit.md`](../audit.md) posait et que son
exécution a tranchées. Elles entrent ici comme faits, avec ce qui les a
tranchées.

| Décision | Marque | Ce qui l'a tranchée |
|---|---|---|
| **La règle du comptage de PD7 s'applique à l'encadrement de détection, et une fonction la satisfait** | **[C]** | Le §4.3, le §6.1 et le §7.3 du traité dérivent le même encadrement de 21 à 31 s, et le code le dérivait **trois fois**. `sim_core::detecteur::encadrement_de_detection` le dérive une fois ; les trois appelants sont des conversions. La règle du comptage porte sur ce qui est **écrit dans le traité** : les trois exemplaires y restent trois, et elle est donc *satisfaite*, non suspendue — ce qui la suspendrait serait d'écrire que le traité n'en compte qu'un. NF-15 tient aussi : les trois tests « 21 à 31 s » et les trois tests de saturation sont inchangés, et le chiffre reste retrouvé par le calcul. **DT6, qui porte sur l'*objet* détecteur, reste posée et non tenue** : deux objets distincts subsistent, `Detecteur` et `DetecteurInfectieux`, et factoriser une dérivation n'est pas les réunir. Pour rouvrir : montrer que le traité n'écrit pas trois fois cet encadrement |
| **Le mode « époque » du push-pull est déclaré non provocable plutôt que retiré** | **[M]** | La branche de divergence d'époque de `agregation.rs` n'a aucun réglage qui l'atteigne : mesuré sur 400 cycles à relance 7, omission 0,30, crash au cycle 11 et partition active, toutes les époques restent égales et aucune rupture `Ligne::Epoque` ne survient, alors que `Ligne4PullPerdu` survient bien — le scénario provoque donc quelque chose, et pas cela. Le test `les_epoques_ne_divergent_jamais` fixe la mesure, et l'entrée est ajoutée à `sim_agents::hors_perimetre()`, qui passe à 21. Retirer la branche aurait supprimé du code correct que le traité décrit ; PD6 veut qu'une absence soit déclarée, pas effacée. Pour rouvrir : un réglage qui fasse diverger deux époques |
