...
Répertoire de rédaction du **Livre II** du compendium *Interopérabilité et Orchestration Agentiques en Entreprise* (Vol. IV). Il ne porte
...
`2 - Compendium/`) et se déclare. *(2)* ⚠ **Rien de ce qui suit n'est levé** : l'écart de portes de ce
Livre — rédigé avant **G-3 et G-4**, cette dernière étant le préalable que le PRD nomme pour lui
précisément — reste écrit, et **G-1 résiduel, G-4 fond, G-5 balayage et G-6 lots sont clos POUR CE SEUL
RÉGIME par dérogation nommée**, leur résidu entier — *la dérogation tombe à la première diffusion et
...
Les pièces sont composées dans [`Compendium.pdf`](../Compendium.pdf) (**1 000 p.**, cible calée le 9 août 2026 ; 1 114 auparavant) : *composer n'est pas
publier.*
...
**aucune** pièce ; la passe n'a produit que des contrôles **mécaniques**, conduits par la même main
...
que personne n'additionne les dérivations. **Ici, la somme des dix cibles a été additionnée avant la
...
| [`16-passeport-agent.md`](16-passeport-agent.md) · [`.html`](16-passeport-agent.html) | Ch. 16 — Le passeport d'agent : synthèse d'un objet encore virtuel | émettre |
...
respecte sa table de couverture. **Six arbitrages de périmètre valent d'être connus, parce qu'ils
cassent une table de couverture si on les défait.**

| Matière | Destination | Régime |
|---|---|---|
| Socle IAM pré-agentique et *zero-trust* | **ch. 3** (Livre I) | **siège amont** — aucun chapitre du Livre II ne le reconstruit |
...
- le **`.html`** — page autonome à thème sombre orange, **sans aucune ressource externe**, prose
  ...
  coupure des mots **sans prévenir**.*

⚠ **Le `.html` est un rendu, pas une seconde source.** Toute correction se fait dans le `.md` puis se
**régénère** :

```bash
python .claude/skills/chapitre-compendium/scripts/rendre-piece.py "2 - Compendium/Livre II/<pièce>.md"
```

```bash
python .claude/skills/chapitre-compendium/scripts/verifier-piece.py "2 - Compendium/Livre II/<pièce>"
```
...
6. **Un siège neuf s'ajoute à la table `SIEGES` de [`PRD/check-sieges.py`](../PRD/check-sieges.py)**,
   la pièce porteuse écrit son marqueur, **et le harnais de mutation se rejoue**.
