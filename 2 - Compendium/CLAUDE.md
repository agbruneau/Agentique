...
⚠ **`TOCAll.md` n'est pas non plus un livrable.** Copie de travail de [`PRD/TOC.md`](PRD/TOC.md) (créée le 25 juillet 2026), augmentée pour les **chapitres 1 à 11 sur 57** (Livres I et II) d'une table des matières détaillée dérivée du texte rédigé des sources — Vol. I *Monographie* §1.0-1.10, §2.x, §3.x et Vol. II *Monographie* ch. 1-4 ; chaque sous-section porte sa provenance et chaque chapitre porte sa table de couverture (décision 6). **Elle n'a pas d'autorité propre** : en cas d'écart avec la ligne Fusion du chapitre dans `PRD/TOC.md`, celle-ci prime. Elle n'est **pas couverte par `check-toc.py`** (hors de son domaine, `PRD/`) et se **désynchronise** à la prochaine passe du TOC si elle n'est pas reportée à la main. Si l'exercice s'étend aux 46 chapitres restants, le repenser comme annexe du TOC plutôt que comme copie parallèle.

⚠ **Ce que la dérivation a fait remonter, et qui appartient au TOC, non à cette copie.** Sept écarts sont consignés dans les blocs concernés de `TOCAll.md`, **signalés et non corrigés** — les porter dans `PRD/TOC.md` relève d'une passe versionnée (bandeau, journal, `check-toc.py`), pas d'un fichier de travail. Deux classes : *(a)* **sections couvertes par un intervalle de la ligne Fusion mais absentes de la phrase « Sections : … »** — §1.2 du Vol. I au ch. 1 (cadres de référence, ISO 11354, EIF/EIRA, modèles de maturité : zéro occurrence dans tout le TOC), §2.8.5 au ch. 4 (déclaré à son départ au ch. 6, jamais à son arrivée), ANP au ch. 8 ; *(b)* **listes de sections non réalignées sur une ligne Fusion corrigée** — « exécution durable » au ch. 1, « modèle de menace, vecteurs d'attaque » au ch. 6 (partis au ch. 20). S'y ajoute une **double affectation** que la décision 2 proscrit : la fusion d'ACP est annoncée par le ch. 8 (Vol. I §3.3.4) *et* par le ch. 10 (Vol. II §3.3), et le §3.4 du ch. 3 du Vol. II est revendiqué par le ch. 10 alors que l'encadré R-8 siège au ch. 7.
