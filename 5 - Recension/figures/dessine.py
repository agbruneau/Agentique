#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Primitives de dessin des figures du rapport de l'art.

Copie de `2 - Compendium/figures/dessine.py`, et non un import : le depot fait
evoluer separement les copies de ses chaines de fabrication (cf. README,
« Les copies du pipeline evoluent separement »). Le corps du rapport fait les
memes 157,0 mm que celui du compendium, donc les memes 445 unites, et les deux
figures de l'ouverture n'emploient que `pile` et `paire`. Les six autres
dispositions sont conservees telles quelles : les retirer ferait de la copie une
variante, ce qui est exactement ce qu'une reprise a l'identique evite.

⚠ UNE SEULE DIVERGENCE, declaree a son endroit : dans `paire`, le second panneau
se cale a la tranche au lieu de suivre l'arrondi de la demi-largeur, faute de
quoi son filet droit sort du viewBox et le panneau se lit comme une boite
ouverte. Le defaut est dans la copie d'origine et n'y est pas corrige — le depot
est clos.

⚠ CONVENTION DE DESSIN, heritee du compendium et non negociable : le viewBox
fait 445 unites de large, soit exactement la largeur du corps (157,0 mm). Une
unite vaut donc UN POINT au rendu, et les corps de texte se posent a leur valeur
finale — 8,5 pour un libelle, 7 pour une annotation. Toute reprise de la grille
de `build/recension.template` oblige a regenerer les figures.

⚠ LE FOND EST EXPLICITE, et il n'est pas decoratif : la figure se rend dans DEUX
supports — le PDF, sur blanc, ou le rectangle est invisible, et une page `.html`
a theme SOMBRE, ou les textes poses hors des bandes seraient noirs sur noir.

⚠ CHAQUE FIGURE PORTE SA RESERVE — ce que la figure refuse de laisser croire.
C'est la discipline du rapport portee a l'image : un rapport derive dont chaque
enonce nomme le regime dont il herite ne peut pas produire une image qui se
tairait sur le sien. `rendu()` refuse une entree sans reserve.
"""
import re
...
def paire(gauche, droite, titre_g=None, titre_d=None, accent_d=True):
    ...
    for k, (panneau, tg, ac) in enumerate(((gauche, titre_g, False), (droite, titre_d, accent_d))):
        # ⚠ SEULE DIVERGENCE DE LA COPIE, et elle est arithmetique : sur un W
        # impair, (W - 26)/2 tombe sur un demi-point, les deux arrondis
        # s'additionnent et le cadre de droite finit a W + 1 — son filet droit
        # sort du viewBox et disparait, le panneau se lisant alors comme une
        # boite ouverte. Le second panneau se cale donc a la tranche.
        x = round(k * (lw + 26)) if k == 0 else W - round(lw)
        y = 0
        if tg:
